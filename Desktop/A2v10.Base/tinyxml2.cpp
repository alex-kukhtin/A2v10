...
void StrPair::SetStr( const wchar_t* str, int flags )
{
	...
	wcsncpy_s(_start, len + 1, str, len + 1);
    _end = _start + len;
                ...
                if ( (_flags & NEEDS_NEWLINE_NORMALIZATION) && *p == CR ) {
                    ...
                    *q = LF;
                    ...
                    if ( *(p+1) == '#' ) {
                        ...
                        int len = 0;
                        wchar_t* adjusted = const_cast<wchar_t*>( XMLUtil::GetCharacterRef( p, buf, &len ) );
                        if ( adjusted == 0 ) {
                            *q = *p;
                            ++p;
                            ++q;
...
void XMLUtil::ToStr(int64_t v, wchar_t* buffer, int bufferSize)
{
	...
	TIXML_SNPRINTF(buffer, bufferSize, L"%lld", (long long)v);
}
...
const wchar_t* XMLDocument::_errorNames[XML_ERROR_COUNT] = {
	...
	L"UNUSED_XML_ERROR_ELEMENT_MISMATCH",
	L"XML_ERROR_PARSING_ELEMENT",
	L"XML_ERROR_PARSING_ATTRIBUTE",
	L"UNUSED_XML_ERROR_IDENTIFYING_TAG",
...
XMLError XMLDocument::Parse( const wchar_t* p, size_t len )
{
    ...
    wcsncpy_s( _charBuffer, len + 1, p, len );
    _charBuffer[len] = 0;
...
void XMLDocument::SetError( XMLError error, const wchar_t* str1, const wchar_t* str2, int lineNum )
{
...
const wchar_t* XMLDocument::GetErrorStr1() const 
{
	return _errorStr1.GetStr();
}

const wchar_t* XMLDocument::GetErrorStr2() const 
{
...
void XMLDocument::PrintError() const
{
    if ( Error() ) {
        ...
        if ( !_errorStr1.Empty() ) {
            TIXML_SNPRINTF( buf1, LEN, L"%s", _errorStr1.GetStr() );
        }
        if ( !_errorStr2.Empty() ) {
            TIXML_SNPRINTF( buf2, LEN, L"%s", _errorStr2.GetStr() );
        }
        ...
        wprintf( L"XMLDocument error id=%d '%s' str1=%s str2=%s line=%d\n",
