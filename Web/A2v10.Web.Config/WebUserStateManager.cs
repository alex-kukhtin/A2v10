...
public class WebUserStateManager : IUserStateManager
{
	...
	const String LicenseCookie = "A2v10.License";
	const String LicensePurpose = "License";
	...
	public void SetLicense(LicenseInfo info)
	{
		...
		json = "sales|purchase|accounting|crm|integrations";
		var key = MachineKey.Protect(Encoding.UTF8.GetBytes(json), LicensePurpose);
		HttpContext.Current.Response.SetCookie(new HttpCookie(LicenseCookie, Convert.ToBase64String(key)));
	}
	...
	public LicenseInfo GetLicense()
	{
		if (HttpContext.Current == null)
			return null;
		var s = HttpContext.Current.Request.Cookies[LicenseCookie];
		if (s == null)
			return null;
		var bytes = MachineKey.Unprotect(Convert.FromBase64String(s.Value), LicensePurpose);
		if (bytes == null)
			return null;
		...
		//JsonConvert.DeserializeObject<LicenseInfo>(json);
	}
}
