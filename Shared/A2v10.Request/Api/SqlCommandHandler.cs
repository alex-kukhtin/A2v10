...
public class SqlCommandHandler : ApiCommandHandler
{
	...
	public override Task<IApiResponse> ExecuteAsync(IApiRequest request)
	{
		...
		{
			SqlCommandAction.LoadModel => LoadModelAsync(request),
			SqlCommandAction.UpdateModel => UpdateModelAsync(request),
			SqlCommandAction.ExecuteSql => ExecuteSqlAsync(request),
			_ => throw new ApiV2Exception($"invalid sql action {_command.Action}"),
		};
	}
