...
namespace A2v10.Stimulsoft
{
	...
	public class StimulsoftReportShim : IStimulsoftReportShim
	{
		...
		public async Task<ExportReportResult> ExportStiReportStreamAsync(Stream input, IStumulsoftReportInfo ri, String targetFormat, Stream output)
		{
			...
			{
				await r.RenderAsync();
				await r.ExportDocumentAsync(StiExportFormat.Pdf, output, StiReportExtensions.GetDefaultPdfSettings());
			...
			{
				await r.RenderAsync();
				await r.ExportDocumentAsync(StiExportFormat.Excel2007, output, StiReportExtensions.GetDefaultXlSettings());
			...
			{
				await r.RenderAsync();
				await r.ExportDocumentAsync(StiExportFormat.Word2007, output, StiReportExtensions.GetDefaultWordSettings());
			...
			{
				await r.RenderAsync();
				await r.ExportDocumentAsync(StiExportFormat.Odt, output, StiReportExtensions.GetDefaultOdtSettings());
			...
			{
				await r.RenderAsync();
				await r.ExportDocumentAsync(StiExportFormat.Ods, output, StiReportExtensions.GetDefaultOdsSettings());
		...
		public async Task<String> ExportDocumentAsync(Stream input, IDataModel dataModel, Stream output)
		{
